    ...
    try:
        ...
        for line in lines:
            if line.startswith("SEVERITY:"):
                ...
                current_key = None
            elif current_key and line.strip() and not any(line.startswith(k) for k in ["SEVERITY:","CONFIDENCE:","ACTION:","ESCALATE:"]):
        ...
        # Full action goes in reasoning for complete display
        full_action = parsed.get("action", "")
        extra = " ".join(reasoning_lines).strip()
        parsed["reasoning"] = (full_action + " " + extra).strip() or "AI analysis via Llama 3.2"
...
def analytics():
    ...
    all_results = query_splunk('index=api_logs earliest=-7d | stats count')
